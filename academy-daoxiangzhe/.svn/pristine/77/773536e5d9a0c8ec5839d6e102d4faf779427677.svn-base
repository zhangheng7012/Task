...
public class CourseController {
    ...
    public Result deleteVideo(@NotNull(message = "视频id不能为空")
        ...
        if(course.getStatus() == 2) {
            ...
            log.error("视频 {} 非下架视频，无法删除", id);
            return new Result(CodeInfo.COURSE_STATUS_WRONG.getCode(), CodeInfo.COURSE_STATUS_WRONG.getMsg());
        }
    }
    ...
    public Result updateVideo(@NotNull(message = "视频id不能为空")
        ...
        Course course1 = courseMapper.selectByPrimaryKey(id);
        // 视频状态，1为上架，上架视频不能更新
        if(course1.getStatus() == 2) {
            course.setId(id);
