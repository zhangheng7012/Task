...
public class ShiroLoginController {
     ...
     */
    @PostMapping("/a/login")
    public Result managerLogin(@Valid Manager manager) {
        ...
        try {
            subject.login(token);
            // 账号的权限集合
            List<String> pers= permissionService.listPermissionURLs(manager.getName());
            // 存入缓存
            jedisUtil.addList(manager.getName(),pers);
