...
import com.dxz.home.utils.Pager;
...
//        因下行代码必须要在查询语句之前，才能进行分页操作，所以这里不适用。
//        PageHelper.startPage(pageNumber,pageSize);
        // 视频集合，含老师信息
//        PageInfo<Course> pageInfo = new PageInfo<Course>(videos);

        Pager<Course> pager = new Pager<>();
        pager.setCurentPageIndex(pageNumber);//页码
        pager.setCountPerpage(pageSize);//每页页数
        pager.setBigList(videos);//需要传入分页的list
//        pager.getRecordCount();//得到总条数
//        pager.getSmallList();//得到小的集合(分页的当前页的记录)
        Map<String,Object> video = new HashMap<>();
        video.put("pageNum",pageNumber);
        video.put("list",pager.getSmallList());
