...
//            log.info("用户 {} 已设置session",users.get(0).getId());
            ...
            if(user.getStatus() == 2){
                log.info("用户 {} 已冻结",user.getId());
                return new Result(CodeInfo.USER_STATUS_FAIL.getCode(), CodeInfo.USER_STATUS_FAIL.getMsg(),users.get(0));
            }
            return new Result(CodeInfo.SUCCESS.getCode(), CodeInfo.SUCCESS.getMsg(),users.get(0));
        }else {
            // 注册
            ...
            User user = userService.addUser(userInfoJSON);
            if (user != null) {
...
//    }
    ...
    public Result getPhoneCode(@NotNull(message = "用户id不能为空") Long id ,@NotBlank(message = "手机号不能为空")
    ...
    public Result bindUserPhone(@NotNull(message = "用户id不能为空") Long id ,@NotBlank(message = "手机号不能为空")
                                @Pattern(regexp = "^[1][3,4,5,6,7,8,9][0-9]{9}$", message = "手机号格式错误") String phone,
        ...
        if(redisCode != null && !"".equals(redisCode)) {
            if (String.valueOf(code).equals(redisCode)) {
                ...
                if( user1.getPhone() == null ){
                    // 将String类型转为Long类型
                    user1.setPhone(Long.valueOf(phone));
                    user1.setScore(user1.getScore() + 20L);
                }else {
                    user1.setPhone(Long.valueOf(phone));
                }
                // 修改用户信息，绑定手机号
    ...
    public Result getEmailCode(@NotNull(message = "用户id不能为空") Long id, @NotBlank(message = "邮箱不能为空")
    ...
    public Result bindUserEmail(@NotNull(message = "用户id不能为空") Long id,
                                @Valid User user,@NotNull(message = "验证码不能为空") Integer code){
        // 取出缓存的验证码
        ...
        if(redisCode != null && !"".equals(redisCode)) {
            if (String.valueOf(code).equals(redisCode)) {
                ...
                Boolean a = userService.updateUser(user1);
                if (a) {
                    log.info("用户 {} 邮箱绑定成功", id);
                    return new Result(CodeInfo.SUCCESS.getCode(), CodeInfo.SUCCESS.getMsg(),user1);
                }else {
                    log.error("用户 {} 邮箱绑定失败", id);
